...
void MovableText::_setupGeometry()
{
    ...
    if (spaceWidth == 0)
        spaceWidth = mpFont->getGlyphAspectRatio(0x4e00) * mCharHeight * 2.0;
    ...
    for (i = mCaption.begin(); i != iend; ++i)
    {
        if (newLine)
        {
            ...
            for (UTFString::iterator j = i; j != iend && *j != '\n'; j++)
            {
                if (*j == ' ')
                    ...
                    len += mpFont->getGlyphAspectRatio((unsigned short)*j) * mCharHeight * 2.0;
            }
            newLine = false;
        }
        ...
        Real horiz_height = mpFont->getGlyphAspectRatio((unsigned short)*i);
        Real u1, u2, v1, v2; 
        Ogre::Font::UVRect utmp;
        utmp = mpFont->getGlyphTexCoords((unsigned short)*i);
        u1 = utmp.left;
        u2 = utmp.right;
        v1 = utmp.top;
        v2 = utmp.bottom;
