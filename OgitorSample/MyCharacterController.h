...
class MyCharacterController : public NxUserRaycastReport
{
public:
	...
	virtual bool onHit(const NxRaycastHit& hits)
	{
		...
		if (!stop && (actor != mController->getActor()) 
			&& actor->userData == (void*)0x00000011 && hits.distance < length)//(hits.distance > length && hits.distance - length <= 30))
			mClosestPoint = hits.worldImpact;
		else if (actor == mController->getActor())
			stop = true;
		return true;
	}

protected:
	virtual 	~MyCharacterController();
