#include "OgrePCZSMSample.h"

void OgrePCZSMSampleApp::chooseSceneManager()
{
	mSceneMgr = mRoot->createSceneManager("PCZSceneManager","PCZSM");
	((PCZSceneManager *)mSceneMgr)->init("ZoneType_Default");
}

void OgrePCZSMSampleApp::addMaterial(const Ogre::String &mat, const Ogre::ColourValue &clr, Ogre::SceneBlendType sbt)
{
	...
	MaterialPtr matPtr = MaterialManager::getSingleton().create(mat,"General");
...
void OgrePCZSMSampleApp::createPoints(Ogre::Vector3 dimensions, Ogre::Vector3 doorDimensions)
{
...
void OgrePCZSMSampleApp::createWalls(Ogre::ManualObject *room, short doorFlags, bool isEnclosure)
{
	if (isEnclosure)
	{
...
void OgrePCZSMSampleApp::createPortals(Ogre::SceneManager *scene, Ogre::ManualObject *room, Ogre::SceneNode *roomNode, Ogre::PCZone *zone, short doorFlags, bool isEnclosure)
{
	...
	if (isEnclosure)
	{
		if(doorFlags & DOOR_FRONT)
		{
			...
			Portal * p = ((PCZSceneManager *)mSceneMgr)->createPortal(portalName);
			p->setCorners(corners);
			p->setNode(roomNode);
			zone->_addPortal(p);
			p->updateDerivedValues();
			roomNode->showBoundingBox(true);
		}
		if(doorFlags & DOOR_BACK)
		{
			// set the corners to the front door corners
...
void OgrePCZSMSampleApp::createPortals(Ogre::SceneManager *scene, Ogre::Entity *room, Ogre::SceneNode *roomNode, Ogre::PCZone *zone, short doorFlags, bool isEnclosure)
{
	Ogre::String portalName;
	Vector3 corners[4];
...
ManualObject * OgrePCZSMSampleApp::createRoom(Ogre::SceneManager *scene,const Ogre::String &name, short doorFlags,bool isEnclosure,Ogre::Vector3 dimensions,Ogre::Vector3 doorDimensions)
{
...
PCZSceneNode * OgrePCZSMSampleApp::createTestBuilding(Ogre::SceneManager *scene, const Ogre::String &name)
{
	...
	// add the room as a child node to the enclosure node
	roomNode = (PCZSceneNode*)exteriorNode->createChildSceneNode( name +"_room3_node", Vector3( 0.0f, 0.0f, -20) );
	roomNode->attachObject(room);
...
void OgrePCZSMSampleApp::createScene()
{
	...
	mCameraMan->getCamera()->getParentSceneNode()->createChildSceneNode()->attachObject(l);
	...
	//SceneNode * lightNode = mCameraMan->getParentSceneNode()->createChildSceneNode("light_Node");
	...
	buildingNode = createTestBuilding(mSceneMgr, String("1"));
	buildingNode->setPosition(0, 0, 0);

	buildingNode = createTestBuilding(mSceneMgr, String("2"));
	buildingNode->setPosition(100, 0, 0);

	mRaySceneQuery = mSceneMgr->createRayQuery(
		Ray(mCameraMan->getCamera()->getPosition(), Vector3::NEGATIVE_UNIT_Y));
	mRaySceneQuery->setSortByDistance(true, 5);
}

void OgrePCZSMSampleApp::frameStarted(const Ogre::FrameEvent &evt)
{

	/*
	static Ray updateRay;
	updateRay.setOrigin(mCameraMan->getCamera()->getPosition());
	updateRay.setDirection(mCameraMan->getCamera()->getOrientation()*Vector3::NEGATIVE_UNIT_Z);
	mRaySceneQuery->setRay(updateRay);

	PCZone * zone = ((PCZSceneNode *)mCameraMan->getCamera()->getParentSceneNode())->getHomeZone();
	((PCZRaySceneQuery *)mRaySceneQuery)->setStartZone(zone);
	((PCZRaySceneQuery *)mRaySceneQuery)->setExcludeNode(mCameraMan->getCamera()->getParentSceneNode());
	RaySceneQueryResult & result = mRaySceneQuery->execute();
	RaySceneQueryResult::iterator i = result.begin();
	if (i != result.end())
	{
	RaySceneQueryResult::reverse_iterator ri = result.rbegin();
	MovableObject * mo = ri->movable;
	if (mTargetMO != mo)
	{
	if (mTargetMO != 0)
	{
	mTargetMO->getParentSceneNode()->showBoundingBox(false);
	}
	mTargetMO = mo;
	mTargetMO->getParentSceneNode()->showBoundingBox(true);
	}
	}*/


	//mCameraNode->translate(mCameraMan->getCamera()->getPosition() - mCameraNode->getPosition(),Node::TS_LOCAL);

	//mCameraMan->getCamera()->getParentSceneNode()->setOrientation(mCameraMan->getCamera()->getOrientation());
}

void OgrePCZSMSampleApp::createCamera()
{
	Camera * camera = mSceneMgr->createCamera("PlayerCam");
	camera->setNearClipDistance(1);
	camera->setFarClipDistance(1000);
	//mCameraNode = (PCZSceneNode *)((PCZSceneManager *)mSceneMgr)->getRootSceneNode()->createChildSceneNode();
	//mCameraNode->attachObject(camera);
	//mCameraNode->showBoundingBox(true);
	//mCameraNode->updateZoneData();
	//mCameraNode->setHomeZone(((PCZSceneManager *)mSceneMgr)->getDefaultZone());
	mCameraMan = new OgreBites::SdkCameraMan(camera);
	mCameraMan->getCamera()->getParentSceneNode()->_updateBounds();
